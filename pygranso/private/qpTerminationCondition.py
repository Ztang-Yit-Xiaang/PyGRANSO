...
class qpTC:
    ...
    def qpTerminationCondition(
        ...
        # obtain # of variables
        torch.numel(gradient_samples[0].F)
        ...
        LB = torch.vstack(
            (
                -torch.ones((q, 1), device=torch_device, dtype=torch_dtype),
                torch.zeros((l + p, 1), device=torch_device, dtype=torch_dtype),
            )
        )
        UB = torch.vstack(
            (
                torch.ones((q, 1), device=torch_device, dtype=torch_dtype),
                mu * torch.ones((l, 1), device=torch_device, dtype=torch_dtype),
                torch.ones((p, 1), device=torch_device, dtype=torch_dtype),
            )
        )
        Aeq = torch.hstack(
            (
                torch.zeros((1, q), device=torch_device, dtype=torch_dtype),
                torch.ones((1, l), device=torch_device, dtype=torch_dtype),
