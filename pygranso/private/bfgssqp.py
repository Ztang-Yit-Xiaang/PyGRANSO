...
from pygranso.private import bfgsDamping as bD
from pygranso.private import linesearchWeakWolfe as lWW
...
class AlgBFGSSQP:
    ...
    def bfgssqp(self, penaltyfn_obj, bfgs_obj, opts, printer, torch_device):
        ...
        bfgssqp:
            ...
            bfgssqp.py (introduced in PyGRANSO v1.0.0)
            Copyright (C) 2016-2021 Tim Mitchell and Buyun Liang

            This file is a MATLAB-to-Python port of bfgssqp.m from
            GRANSO v1.6.4 with the following new functionality and/or changes:
                1. Search Direction Rescaling
                2. Disable termination code 6 to allow further progress in deep learning
        ...
        self.bfgs_update_fn = lambda s, y, sty, damped: self.bfgs_obj.update(
            s, y, sty, damped
        )
        ...
        [stat_vec, self.stat_val, qps_solved, _, _] = (
        ...
        self.linesearch_fn = lambda x, f, g, p, ls_maxit: lWW.linesearchWeakWolfe(
            ...
            g,
            p,
            lambda x_in, get_grad=True: self.penaltyfn_obj.evaluatePenaltyFunction(
                x_in, get_grad
            ),
            wolfe1,
            wolfe2,
            ...
            step_tol,
            self.init_step_size,
            self.linesearch_maxit,
            self.is_backtrack_linesearch,
            self.torch_device,
        )
    ...
    def bumpFallbackLevel(self):
        ...
        if can_fallback:
            ...
            self.fallback_level == self.LAST_FALLBACK_LEVEL
            and self.random_attempts < self.max_random_attempts
        ):
            can_fallback = True
    ...
    def linesearchNondescent(self, x, f, g, p):
        ...
        [alpha, x, f, g, fail] = self.linesearch_fn(
            x, f, g, p, self.linesearch_nondescent_maxit
        )
        fail = 0 + 3 * (fail > 0)
    ...
    def computeApproxStationarityVector(self):
        ...
        qPTC_obj = qpTC()
        try:
            [stat_vec, n_qps, ME] = qPTC_obj.qpTerminationCondition(
                self.penaltyfn_at_x,
                grad_samples,
                self.apply_H_QP_fn,
                self.QPsolver,
    ...
    def getApplyHRegularized(self):
        ...
        [Hr, code] = rPDM.regularizePosDefMatrix(
            self.bfgs_obj.getState(),
            self.regularize_threshold,
